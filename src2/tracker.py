...
class Tracker:
    def __init__(self, cfg) -> None:
        ...
        if self.cfg.MODEL.MODE == 'test':
            self.output_dir = osp.join(self.cfg.OUTPUT.INFERENCE_DIR,
                                       f'test-{self.cfg.DATASET.NAME}-{self.cfg.DATASET.SEQUENCE[0]}-{int(time.time())}')
            self.tracklet = Tracklet(self.cfg, self.output_dir)
        else:
            self.output_dir = osp.join(self.cfg.OUTPUT.CKPT_DIR,
                                       f'train-{self.cfg.DATASET.NAME}-{self.cfg.DATASET.SEQUENCE[0]}-{self.cfg.SOLVER.TYPE}-{int(time.time())}')
            # self.tracklet = Tracklet(self.cfg, self.output_dir)
    ...
    def load_dataset(self):
        ...
        dataset = [BaseGraphDataset(self.cfg, self.cfg.DATASET.SEQUENCE, mode, self.feature_extractor,
                                    osp.join(self.cfg.DATASET.DIR, dataset_name))
    ...
    def train(self):
        ...
        scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optim, self.cfg.SOLVER.EPOCHS, eta_min=0.00005)
        scheduler_warmup = GradualWarmupScheduler(optim, 1.0, 10, scheduler_cosine)
        optim.zero_grad()
        optim.step()

        train_dataset, eval_dataset = self.load_dataset()
        train_loader = DataLoader(train_dataset, self.cfg.SOLVER.BATCH_SIZE, shuffle=True, collate_fn=udf_collate_fn,
                                  drop_last=True)
        eval_loader = DataLoader(eval_dataset, self.cfg.SOLVER.BATCH_SIZE, collate_fn=udf_collate_fn)
        # test_loader = DataLoader(test_dataset, self.cfg.SOLVER.BATCH_SIZE, collate_fn=udf_collate_fn)
        ...
        for epoch in range(self.cfg.SOLVER.EPOCHS):
            ...
            # if epoch % self.cfg.SOLVER.EVAL_EPOCH_MOT == 0:
            #     visualize_dir = None
            #     if self.cfg.OUTPUT.VISUALIZE:
            #         visualize_dir = osp.join(self.output_dir, 'visualize')
            #         make_dir(visualize_dir)
            #     self._test_one_epoch(test_loader, self.cfg.SOLVER.MAX_PASSING_STEPS,
            #                          visualize_dir)  # generate inference file
    ...
    def _train_one_epoch(self, epoch: int, dataloader, optimizer, scheduler, writer):
        ...
        for i, data in enumerate(dataloader):
            ...
            for graph, node_feature, edge_feature, y_true in data:
                ...
                for _ in range(self.cfg.SOLVER.MAX_PASSING_STEPS):
                    ...
                    # Compute accuracy for each graph
                    graph_accuracy = torchmetrics.functional.accuracy(y_pred, y_true, task='binary')
            ...
            epoch_accuracy = sum(graph_accuracies) / len(graph_accuracies)
            ...
            if self.cfg.OUTPUT.LOG:
                logger.info(f"epoch=({epoch}/{self.cfg.SOLVER.EPOCHS - 1})"
                            ...
                            f" | graph_accuracy={epoch_accuracy:.4f}")
        ...
        writer.add_scalar("Accuracy/train", avg_accuracy, epoch)  # Add accuracy to tensorboard

        if self.cfg.OUTPUT.LOG:
            logger.opt(ansi=True).info(
                f"<fg 255,204,0>【Finished training epoch {epoch}】 avg_train_loss={avg_loss:.4f} avg_train_accuracy={avg_accuracy:.4f}</fg 255,204,0>")
    ...
    def _eval_one_epoch(self, epoch: int, dataloader, writer, output_dir):
        ...
        writer.add_scalar("Accuracy/validation", avg_accuracy, epoch)  # Add accuracy to tensorboard

        if self.cfg.OUTPUT.LOG:
            logger.opt(ansi=True).info(
                f"<fg 255,153,51>【Finished eval epoch {epoch}】 avg_eval_loss={avg_loss:.6f} avg_eval_accuracy={avg_accuracy:.4f}</fg 255,153,51>")
    ...
    def _test_one_epoch(self, dataloader, max_passing_steps: int, visualize_output_dir=None):
        ...
        for i, data in enumerate(dataloader):
            ...
            if self.cfg.OUTPUT.LOG:
                logger.opt(ansi=True).info(
                    f'<fg 255,153,51>【Finished inference iteration {i}/{len(dataloader) - 1}】</fg 255,153,51>\n')
